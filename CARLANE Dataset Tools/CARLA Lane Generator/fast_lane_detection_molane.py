...
from scripts.label_saver_molane import LabelSaver
...
class CarlaGame(object):
    ...
    def __init__(self):
        ...
        if cfg.is_third_person:
            ...
            self.camera_transforms = [carla.Transform(carla.Location(x=0.29, z=0.72), carla.Rotation(pitch=-4.2)),
                                      carla.Transform(carla.Location(x=0.35, z=0.68), carla.Rotation(pitch=-3.9)),
                                      carla.Transform(carla.Location(x=0.31, z=0.76), carla.Rotation(pitch=-4.4))]
    ...
    def save_data(self, image_rgb, x_lanes_list, curve_type):
        if (cfg.CARLA_TOWN == 'Town04' or cfg.CARLA_TOWN == 'Town04_Opt' or cfg.CARLA_TOWN == 'Town06' or cfg.CARLA_TOWN == 'Town06_Opt') \
                and 'val' not in cfg.suffix:
            if (curve_type == 'steep_left_curve/' or curve_type == 'steep_right_curve/') and \
                    self.curve_counters['steep_left_curve/'] + self.curve_counters['steep_right_curve/'] < cfg.max_images_per_class * 2:
...
class VehicleManager(object):
    ...
    def detect_curve(self, vehicle, waypoint_list):
        ...
        if angle < lane_angle['straight']:
            ...
            if cross_product < 0:
                self.curve_type = 'steep_left_curve/'
                return True
            elif cross_product > 0:
                self.curve_type = 'steep_right_curve/'
                return True
            else:
                self.curve_type = 'straight/'
                return False
    ...
    def move_agent(self, vehicle, waypoint_list, waypoints_visited):
        ...
        if self.detect_curve(vehicle, waypoint_list):
            self.oscillation = 0.9
        else:
            self.oscillation = 1.2
