...
class LaneDatasetTrainPseudo(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        cls_conf, domain_label, disc_conf = l_info[-3], l_info[-2], l_info[-1]
        cls_conf = torch.tensor(float(cls_conf))
        domain_label = torch.tensor(int(domain_label))
        disc_conf = torch.tensor(float(disc_conf))
        ...
        if self.use_aux:
            return image, cls_label, seg_label, cls_conf, domain_label, disc_conf

        return image, cls_label, cls_conf, domain_label, disc_conf
