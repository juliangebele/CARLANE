...
def get_optimizer(net, cfg, lr):
    training_params = filter(lambda p: p.requires_grad, net.parameters())
    if cfg.optimizer == 'Adam':
        optimizer = torch.optim.Adam(training_params, lr=lr, betas=(0.5, 0.999), weight_decay=cfg.weight_decay)
    elif cfg.optimizer == 'SGD':
        optimizer = torch.optim.SGD(training_params, lr=lr, momentum=cfg.momentum, weight_decay=cfg.weight_decay)
    else:
