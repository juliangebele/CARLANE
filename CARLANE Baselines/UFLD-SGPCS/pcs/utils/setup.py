...
def check_pretrain_dir(config_json):
    ...
    if "pretrained_exp_dir" in config_json and config_json["pretrained_exp_dir"] is not None:
...
def process_config(config_json, override_dotmap=None):
    ...
    if config.debug:
        ...
        if config.continue_exp_dir is not None and isinstance(config.continue_exp_dir, str):
            ...
            exp_dir = os.path.join(exp_base, "experiments", config.exp_name, config.exp_id)
...
def setup_logging(log_dir):
    log_file_format = "[%(levelname)s] %(asctime)s: %(message)s in %(pathname)s:%(lineno)d"
...
def prepare_dirs(config):
    exp_dir = os.path.join(config.exp_base, "experiments", config.exp_name, config.exp_id)
    ...
    if config.pretrained_exp_dir is None or config.copy_exp_dir is False:
...
def get_cmd(config, script_path="/rscratch/xyyue/anaconda3/envs/ssda2/bin/python ./run.py"):
