...
def get_metric_dict(griding_num, num_lanes):
    metric_dict = {
        'name': ['top1', 'top2', 'top3', 'iou'],
        'op': [MultiLabelAcc(), AccTopk(griding_num, 2), AccTopk(griding_num, 3), Metric_mIoU(num_lanes + 1)],
...
def update_metrics(metric_dict, pair_data, use_aux):
    if use_aux:
        ...
        for i in range(len(metric_dict['name']) - 1):
            metric_op = metric_dict['op'][i]
            data_src = metric_dict['data_src'][i]
            metric_op.update(pair_data[data_src[0]], pair_data[data_src[1]])
...
if __name__ == '__main__':
    ...
    me = AccTopk(0, 5)
    me.update(converter(b), converter(a))
    # me.update(b, a)
    print(me.get())
