...
def get_optimizer(encoder, classifier, cfg):
    training_params = filter(lambda p: p.requires_grad, list(encoder.parameters()) + list(classifier.parameters()))
...
def get_optimizer_disc(discriminator, cfg):
    training_params = filter(lambda p: p.requires_grad, discriminator.parameters())
    if cfg.optimizer == 'Adam':
        optimizer = torch.optim.Adam(training_params, lr=cfg.learning_rate_disc, weight_decay=cfg.weight_decay)
    elif cfg.optimizer == 'SGD':
        optimizer = torch.optim.SGD(training_params, lr=cfg.learning_rate_disc, momentum=cfg.momentum, weight_decay=cfg.weight_decay)
    else:
        raise NotImplementedError
    return optimizer
...
def get_loss_dict(cfg):
    if cfg.use_aux:
        loss_dict = {
            ...
            'weight': [1.0, cfg.sim_loss_w, 1.0, cfg.shp_loss_w, cfg.domain_loss_w],
        ...
        loss_dict = {
            ...
            'weight': [1.0, cfg.sim_loss_w, cfg.shp_loss_w, cfg.domain_loss_w],
